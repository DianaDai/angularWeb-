 ...
 */
function ngGridCtrl($scope,$state,apiService) {
    ...
    function loadData() {
        $scope.ngData=[];
        apiService.get('http://localhost:55472/api/Employees/Get', null,
            function(result){
                if(result.data.length>0) {
                    angular.forEach(result.data, function (value) {
                       /* var temp = {
                            ...
                            Name:value.Name,
                            Age:value.Age,
                            Position:value.Department,
                            Status:value.Code,
                            Date:value.EntryTime
                        }*/
                        $scope.ngData.push(value);
                    })
                }
                console.log(result)
            },
       ...
       /* $scope.resultValue = apiService.get({Id: "5"});//Get(string id)
    ...
    $scope.gridOptions = {
        ...
        columnDefs: [
            ...
            },
            {
                ...
                sortable: false,
                pinnable: false,
                cellTemplate:'<div class="cellDetail" ng-click="foo(row)">详情</div>'
            }
        ],
        enablePaging: true,
        showFooter: true,
    ...
    $scope.deleteItem = function(){
        if($scope.gridOptions.$gridScope.selectedItems.length>0) {
            ...
            apiService.get('http://localhost:55472/api/Employees/Delete/', config,
                function (result) {
                    loadData();
                    console.log(result)
                },
    ...
    $scope.editItem = function(){
        ...
        $scope.editData = {
            Id: $scope.gridOptions.$gridScope.selectedItems[0].Id,
            Name:$scope.gridOptions.$gridScope.selectedItems[0].Name,
            Age: Number($scope.gridOptions.$gridScope.selectedItems[0].Age),
            Department: $scope.gridOptions.$gridScope.selectedItems[0].Department,
            Code: $scope.gridOptions.$gridScope.selectedItems[0].Code,
            EntryTime: new Date($scope.gridOptions.$gridScope.selectedItems[0].EntryTime)
        }
        //$scope.editData = $scope.gridOptions.$gridScope.selectedItems[0];
        $state.go('tables.edit_employee', {
...
function addCtrl($scope,$state,apiService){
    $scope.newData = {
        Name: null,
        Age: null,
        Department: null,
    ...
    $scope.AddItem = function(){
        apiService.post('http://localhost:55472/api/Employees/Post/',$scope.newData,
            function(result){
                var truthBeTold = window.confirm("单击“确定”继续");
                if (truthBeTold) {
                    $state.go('tables.nggrid');
                }
                console.log(result)
            },
            function(response){
...
function editCtrl($scope,$stateParams,$state,apiService){
    $scope.params = $stateParams.params;
    console.log($scope.params);
    $scope.editData = $scope.params;
    $scope.updateItem = function(){
        var config = {
            params: {
                Id: $scope.editData.Id,
                employee:$scope.editData
            }
        };
        apiService.post('http://localhost:55472/api/Employees/Update/',$scope.editData,
            function(result){
                var truthBeTold = window.confirm("单击“确定”继续");
                if (truthBeTold) {
                    $state.go('tables.nggrid');
 ...
 */
function notifyCtrl($timeout,$scope, notify) {
...
angular
    ...
    .controller('modalDemoCtrl', ['$scope', '$uibModal',modalDemoCtrl])
    .controller('ionSlider', ionSlider)
    .controller('wizardCtrl', wizardCtrl)
    .controller('CalendarCtrl', CalendarCtrl)
    .controller('chartJsCtrl', chartJsCtrl)
    .controller('GoogleMaps', GoogleMaps)
    .controller('ngGridCtrl', ['$scope','$state','apiService',ngGridCtrl])
    ...
    .controller('editCtrl', ['$scope','$stateParams','$state','apiService',editCtrl])
    .controller('addCtrl', ['$scope','$state','apiService',addCtrl])
